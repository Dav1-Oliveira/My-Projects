const gridWidth = 10
const shapeFreezeAudio = new Audio('audios/shapeFreeze.wav');
shapeFreezeAudio.volume = 0.1;

const completeLineAudio = new Audio('audios/lineComplete.wav');
completeLineAudio.volume = 0.1;
...
function moveLeft() {
  isEdgeLimit = currentShape.some(squareIndex => (squareIndex + currentPosition) % gridWidth === 0)
...
function moveRight() {
  isEdgeLimit = currentShape.some(squareIndex => (squareIndex + currentPosition) % gridWidth === gridWidth - 1)
  if (isEdgeLimit) return
